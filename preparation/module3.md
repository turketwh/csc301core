# CSC 301: Preparation Guide For Module 3

## Read And Watch
Read the following chapters of the *Course Reader* linked from *Course Administration* module of the course Canvas site. 

**By Friday, September 12, 2025:**

* Chapter 10: Recurrences: Master Theorem
* Chapter 11: Recurrences: Master Theorem: Activities Introduction

The major points to focus on in this part of the reading are:

* The *categories* of recurrence relations the Master Theorem can be applied to
* The components of a recurrence relation that are needed to make use of the Master Theorem
* Selecting the appropriate case of the Master Theorem based on the recurrence relation components

**By Wednesday, September 17, 2025:**  *(this reading, not yet available, will be posted by Sunday, September 14, 2025)*

* Chapter 12: Recurrences: Recurrence Trees
* Chapter 13: Recurrences: Recurrence Trees: Activities Introduction

The major points to focus on in this part of the reading are:

* How a recurrence tree represents the decomposition of a problem and the work required at each level of the decomposition
* Determining the height of a recurrence tree
* Determining the total amount of work specified by a recurrence tree

There may be a few videos linked in the reading that should be watched.  These will be annotated in a special callouts in the reading, each labeled as *Video Resource*.

## Questions And Clarification
If you have questions you would like to ask about the material in the reading, submit them though the *Questions And Clarification* Google form linked from *Module 3* in the course Canvas site.
